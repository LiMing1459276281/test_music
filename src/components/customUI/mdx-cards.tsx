...
export default function MdxCards({lang,moreLabel,posts}: Props) {
            ...
            {posts.map((item, index) => (
                <div key={index} className="flex flex-col w-full p-1.5 relative overflow-hidden rounded-lg border border-gray-200 dark:bg-gray-800 dark:border-gray-700 shadow-md max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl">
                    <div className="flex flex-col items-stretch justify-start p-3 h-full gap-1">
                        ...
                        <div className="flex">
                            {/* Title */}
                            <Link href={`${getPathname(lang,`/blog/article/${item.slug}`)}`} className="text-xl font-bold text-gray-800 dark:text-slate-100">
                    ...
                    <div>
                        {/* Button */}
                        <a href={`${getPathname(lang,`/blog/article/${item.slug}`)}`} className="h-9 font-medium text-sm flex items-center justify-center rounded-md duration-200 cursor-pointer text-gray-500 dark:text-gray-200 bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 dark:hover:text-gray-100 hover:bg-gray-300 hover:text-gray-900">{moreLabel}</a>
                    </div>
                </div>
            ))}
