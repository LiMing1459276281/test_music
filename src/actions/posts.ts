...
export async function getRecentBlogList(lang:string="en"){
    if(!lang){
        lang = "en";
    }
  
    const root = process.cwd();
    let files = fs.readdirSync(path.join(root, 'public',"blog",lang)); // get the files
    files = files.filter(file => file.split('.')[1] == "mdx"); // filter only the mdx files
    const posts = files.map(file => { // for each file extract the front matter and the slug
        const fileData = fs.readFileSync(path.join(root, 'public',"blog",lang,file),'utf-8');
        const {data} = matter(fileData);
       return {
            frontMatter:data as FrontMatter,
            slug:file.split('.')[0]
